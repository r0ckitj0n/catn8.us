...
catn8_rate_limit_require('mystery.csi_live_bootstrap.' . $viewerId, 20, 600);
...
$scenarioRow = Database::queryOne(
    'SELECT s.id, s.game_id, s.title, s.specs_json, g.owner_user_id, g.mystery_id, s.csi_detective_entity_id
...
$csiEntityId = (int)($scenarioRow['csi_detective_entity_id'] ?? 0);
if ($csiEntityId <= 0) {
    catn8_json_response(['success' => false, 'error' => 'CSI Detective is not configured for this scenario'], 404);
}

$csiRow = Database::queryOne(
    'SELECT se.entity_id, se.override_json, e.slug, e.name, e.data_json
     ...
     LIMIT 1',
    [$scenarioId, $csiEntityId]
);
if (!$csiRow) {
    catn8_json_response(['success' => false, 'error' => 'CSI Detective is not attached to this scenario'], 404);
}

$csiEntityId = (int)($csiRow['entity_id'] ?? 0);

require_once __DIR__ . '/interrogate_functions.php';
$interrogationState = catn8_interrogate_load_state($scenarioId, $csiEntityId, $viewerId, $isAdmin);

$model = trim((string)($settings['csi_live']['model'] ?? 'gemini-2.0-flash-exp'));
$baseInstruction = (string)($settings['csi_live']['system_instruction'] ?? 'You are a CSI Detective assisting in an active murder investigation. You are calm, analytical, and evidence-driven. Stay in character.');

$scenarioTitle = $interrogationState['scenario_title'];
$csiName = $interrogationState['entity_name'];
...
if (!is_string($resp) || $resp === '') {
    catn8_log_error('Gemini ephemeral token request failed (csi)', ['http_status' => $status]);
...
if (!is_array($data)) {
    catn8_log_error('Gemini ephemeral token response was not valid JSON (csi)', ['http_status' => $status, 'raw' => substr((string)$resp, 0, 2000)]);
...
if ($status >= 400 || isset($data['error'])) {
    ...
    catn8_log_error('Gemini ephemeral token error (csi)', ['http_status' => $status, 'message' => $msg, 'raw' => substr((string)$resp, 0, 2000)]);
