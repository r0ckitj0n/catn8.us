...
- **Table Names:** Must be **plural snake_case** (e.g., `suspects`, `game_logs`, `order_items`).
- **Column Names:** Must be **snake_case** (e.g., `is_active`, `created_at`, `user_id`).
- **Foreign Keys:** Must follow the pattern `[singular_table]_id` (e.g., `game_id` references `games.id`).
- **Booleans:** Use `TINYINT(1)` for booleans. Prefix columns with `is_` or `has_` (e.g., `is_completed`).
