...
interface Accumul8PayBillsTabProps {
  ...
  savePayBillRow: (row: Accumul8Transaction, draftOverride?: LedgerInlineDraft) => Promise<void>;
...
export function Accumul8PayBillsTab({
  ...
  const flushAmountSave = React.useCallback((billTx: Accumul8Transaction, amount: number) => {
    void savePayBillRow(billTx, { ...(payBillDraftById[billTx.id] || {}), amount });
  }, [payBillDraftById, savePayBillRow]);
        ...
        <table ref={payBillsTableRef} className="table table-sm accumul8-table accumul8-table--measured accumul8-table--pay-bills accumul8-sticky-head" style={payBillsTable.tableStyle}>
          ...
          <tbody>
            {payBillsTable.rows.map((billTx) => {
              ...
              return (
                <tr
                  ref={(node) => setInlineRowRef(`paybill-${billTx.id}`, node)}
                  key={billTx.id}
                  className={['accumul8-list-item', activePayBillRowId === billTx.id ? 'is-editing' : '', payBillDraftById[billTx.id] ? 'has-draft' : ''].filter(Boolean).join(' ')}
                >
                  ...
                  <td className="text-end">{activePayBillRowId === billTx.id ? <input className="form-control form-control-sm accumul8-month-table-input" type="number" step="0.01" value={payBillDraftById[billTx.id]?.amount ?? billTx.amount} onChange={(e) => { const amount = Number(e.target.value); setPayBillRowDraft(billTx, { amount }); }} onBlur={(e) => { if (busy) return; const amount = Number(e.currentTarget.value); if (!Number.isFinite(amount)) return; setPayBillRowDraft(billTx, { amount }); flushAmountSave(billTx, amount); }} onKeyDown={(e) => { if (e.key === 'Enter' && !busy) { e.preventDefault(); const amount = Number(e.currentTarget.value); if (!Number.isFinite(amount)) return; setPayBillRowDraft(billTx, { amount }); flushAmountSave(billTx, amount); } }} disabled={busy || !billEditPolicy.canEditCoreFields} /> : <button type="button" className="accumul8-inline-cell-trigger accumul8-inline-cell-trigger--numeric" onClick={() => activatePayBillRow(billTx.id)} disabled={busy}>{Number(billTx.amount || 0).toFixed(2)}</button>}</td>
                  <td>
                  ...
                  <td className="text-end">
                    <div className="accumul8-row-actions accumul8-row-actions--always-on">
                      ...
                      <button type="button" className={`btn btn-sm btn-outline-primary accumul8-icon-action${flashingSaveButtonKey === `paybill-${billTx.id}` ? ' is-flashing' : ''}`} onClick={() => void savePayBillRow(billTx, payBillDraftById[billTx.id])} disabled={busy || !payBillDraftById[billTx.id]} aria-label={`Save ${billTx.description}`} title={payBillDraftById[billTx.id] ? `Save ${billTx.description}` : `No changes to save for ${billTx.description}`}><span aria-hidden="true">{ACCUMUL8_SAVE_BUTTON_EMOJI}</span></button>
                    </div>
                  </td>
                </tr>
              );
