...
Database::execute(
    'INSERT INTO build_wizard_steps (project_id, step_order, phase_key, step_type, title, source_ref)
     VALUES (?, 1, ?, ?, ?, ?), (?, 2, ?, ?, ?, ?)',
    [
        $oldProjectId, 'land_due_diligence', 'documentation', 'Complete boundary and topographic survey', 'Cabin Timeline.xlsx',
        $oldProjectId, 'framing_shell', 'construction', 'Frame walls and install cabin gable', 'Cabin Timeline.xlsx',
    ]
);
...
assert_true((int)$repair['remapped_step_ids'] === 3, 'repair should remap leftover step ids onto the new project steps');
