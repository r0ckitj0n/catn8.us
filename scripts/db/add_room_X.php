<?php
// Dev utility: Seed room 'X' (Settings) into DB and ensure a default map
...
try {
    ...
    // 1) Ensure room_settings row for X
    $existing = Database::queryOne("SELECT id, room_number FROM room_settings WHERE room_number = 'X' LIMIT 1");
    if ($existing) {
        // Update friendly defaults if missing
        Database::execute(
            "UPDATE room_settings 
             SET room_name = COALESCE(NULLIF(room_name,''), 'Settings'),
                 door_label = COALESCE(NULLIF(door_label,''), 'Settings'),
        ...
        Database::execute(
            "INSERT INTO room_settings (room_number, room_name, door_label, description, display_order, is_active) 
             VALUES ('X', 'Settings', 'Settings', 'Admin and configuration page.', 98, 1)"
        );
    }
    ...
    // 2) Ensure map exists and seed an 'Original' active map for X

    $orig = Database::queryOne("SELECT id FROM room_maps WHERE room_number = 'X' AND map_name = 'Original' LIMIT 1");
    if ($orig) {
        // Ensure it's active and coordinates present
        Database::execute("UPDATE room_maps SET is_active = TRUE WHERE id = ?", [$orig['id']]);
        Database::execute("UPDATE room_maps SET is_active = FALSE WHERE room_number = 'X' AND id <> ?", [$orig['id']]);
    } else {
        Database::execute(
            "INSERT INTO room_maps (room_number, map_name, coordinates, is_active) VALUES ('X', 'Original', ?, TRUE)",
            [json_encode([])]
        );
        $newId = Database::lastInsertId();
        Database::execute("UPDATE room_maps SET is_active = FALSE WHERE room_number = 'X' AND id <> ?", [$newId]);
    }

    respond(true, ['message' => "Room 'X' ensured in room_settings and room_maps"]);
} catch (Exception $e) {
