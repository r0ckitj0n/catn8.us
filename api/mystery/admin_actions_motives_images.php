<?php
if ($action === 'upload_motive_image') {
    catn8_mystery_require_admin($isAdmin);
    catn8_require_method('POST');

    $id = (int)($_POST['motive_id'] ?? 0);
    if ($id <= 0) catn8_json_response(['success' => false, 'error' => 'motive_id is required'], 400);
    $row = Database::queryOne('SELECT id FROM mystery_motives WHERE id = ? LIMIT 1', [$id]);
    ...
    $fileName = 'motive_' . (string)$id . '.' . $ext;
    $dest = $dir . '/' . $fileName;
    ...
    Database::execute(
        'INSERT INTO mystery_motive_images (motive_id, title, url, alt_text, prompt_text, negative_prompt_text, provider, model)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE url = VALUES(url)',
        [$id, '', $url, '', '', '', '', '']
    );
    $img = catn8_mystery_motive_image_load($id);
    catn8_json_response(['success' => true, 'motive_id' => $id, 'url' => $url, 'image' => $img]);
}

if ($action === 'delete_motive_image') {
    catn8_mystery_require_admin($isAdmin);
    catn8_require_method('POST');
    $body = catn8_read_json_body();

    $id = (int)($body['motive_id'] ?? 0);
    if ($id <= 0) catn8_json_response(['success' => false, 'error' => 'motive_id is required'], 400);
    $row = Database::queryOne('SELECT id FROM mystery_motives WHERE id = ? LIMIT 1', [$id]);
    if (!$row) catn8_json_response(['success' => false, 'error' => 'Motive not found'], 404);
    ...
    Database::execute(
        'UPDATE mystery_motive_images SET url = ? WHERE motive_id = ? LIMIT 1',
        ['', $id]
    );
    $img = catn8_mystery_motive_image_load($id);
    catn8_json_response(['success' => true, 'motive_id' => $id, 'deleted' => $deleted, 'image' => $img]);
}
