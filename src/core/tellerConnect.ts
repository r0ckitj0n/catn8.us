import { createTellerMessageHandler, startTellerIframeTracking } from './tellerConnectUtils';
...
export async function openTellerConnect(
  ...
  return await new Promise((resolve, reject) => {
    ...
    const messageHandler = createTellerMessageHandler(emitEvent);
    window.addEventListener('message', messageHandler);
    const stopIframeTracking = startTellerIframeTracking(emitEvent);
