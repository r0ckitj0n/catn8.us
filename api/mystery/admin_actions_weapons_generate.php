<?php
/**
 * admin_actions_weapons_generate.php - AI weapon generation
...
if ($action === 'generate_weapon') {
    ...
    if ($id > 0) {
        $row = Database::queryOne('SELECT id, name, description FROM mystery_weapons WHERE id = ? LIMIT 1', [$id]);
    ...
    $systemPrompt = trim(
        'You generate missing fields for a weapon record for a detective mystery game.' . "\n" .
    ...
    $userPrompt = trim('Current weapon fields JSON:' . "\n" . $curJson);
    ...
    try {
        ...
        if ($id > 0) {
            Database::execute(
                'UPDATE mystery_weapons SET description = ? WHERE id = ? LIMIT 1',
                [$cur['description'], $id]
            );
        }
        ...
        if ($withImage) {
            ...
            if ($imgProvider === 'google_vertex_ai') {
                ...
                $prompt = catn8_mystery_weapon_build_image_prompt($cur);
                ...
                $fileName = 'weapon_' . $id . '.png';
                $dest = $dir . '/' . $fileName;
                file_put_contents($dest, base64_decode($b64));
                
                $url = '/images/mystery/' . $fileName;
                Database::execute(
                    'INSERT INTO mystery_weapon_images (weapon_id, url, title, prompt_text, provider, model) 
                     VALUES (?, ?, ?, ?, ?, ?) 
            ...
            // Add OpenAI DALL-E 3 support here if needed later
        }

        $img = catn8_mystery_weapon_image_load($id);
        
        $rowOut = $id > 0
            ? Database::queryOne('SELECT id, slug, name, description, is_archived, created_at, updated_at FROM mystery_weapons WHERE id = ? LIMIT 1', [$id])
            : null;
