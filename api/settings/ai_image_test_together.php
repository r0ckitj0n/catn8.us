<?php
if ($provider === 'together_ai' || $provider === 'fireworks_ai') {
    $apiKey = secret_get(catn8_settings_ai_image_secret_key($provider, 'api_key'));
    if (!is_string($apiKey) || trim($apiKey) === '') {
        $fail(500, 'Missing AI image API key (' . $provider . ')');
    }
    if ($baseUrl === '') {
        $fail(500, 'Missing base_url in AI Image config for provider ' . $provider);
    }

    $root = rtrim(catn8_validate_external_base_url($baseUrl), '/');
    $url = preg_match('#/v1$#', $root) ? ($root . '/images/generations') : ($root . '/v1/images/generations');
    ...
    $res = catn8_http_json_with_status('POST', $url, ['Authorization' => 'Bearer ' . trim((string)$apiKey)], $body);
    ...
    if (!$ok) {
        $fail(500, 'Image provider test response missing image data', ['http_status' => $status]);
    }
