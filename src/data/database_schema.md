...
- `support_phone` (VARCHAR(32), DEFAULT `''`)
- `support_email` (VARCHAR(191), DEFAULT `''`)
- `routing_number` (VARCHAR(32), DEFAULT `''`)
...
- `bank_connection_id` (INT, NULLABLE, FOREIGN KEY -> accumul8_bank_connections.id)
- `provider_name` (VARCHAR(32), default `''`)
- `teller_account_id` (VARCHAR(191), NULLABLE)
- `teller_enrollment_id` (VARCHAR(191), NULLABLE)
- `teller_sync_anchor_date` (DATE, NULLABLE)
- `teller_backfill_cursor_id` (VARCHAR(191), NULLABLE)
- `teller_backfill_complete` (TINYINT(1), DEFAULT 0)
- `teller_history_start_date` (DATE, NULLABLE)
- `teller_history_end_date` (DATE, NULLABLE)
- `account_name` (VARCHAR(191))
- `account_nickname` (VARCHAR(191), DEFAULT `''`)
- `account_type` (VARCHAR(40), default `checking`)
- `account_subtype` (VARCHAR(64), DEFAULT `''`)
- `institution_name` (VARCHAR(191))
- `account_number_mask` (VARCHAR(32), DEFAULT `''`)
- `mask_last4` (VARCHAR(8))
- `routing_number` (VARCHAR(32), DEFAULT `''`)
- `currency_code` (VARCHAR(3), DEFAULT `USD`)
- `statement_day_of_month` (TINYINT, NULLABLE)
- `payment_due_day_of_month` (TINYINT, NULLABLE)
...
- `source_kind` (VARCHAR(24), e.g. `manual`/`recurring`/`teller`)
...
- `id` (INT, PRIMARY KEY, AUTO_INCREMENT)
- `owner_user_id` (INT, FOREIGN KEY -> users.id)
- `provider_name` (VARCHAR(32), default `teller`)
- `institution_id` (VARCHAR(64), NULLABLE)
- `institution_name` (VARCHAR(191), NULLABLE)
- `teller_enrollment_id` (VARCHAR(191), NULLABLE)
- `teller_user_id` (VARCHAR(191), NULLABLE)
- `teller_access_token_secret_key` (VARCHAR(191), NULLABLE)
...
- `catalog_trace_json` (LONGTEXT/JSON, NULLABLE, durable OCR/catalog snapshot including raw OCR text, page catalog, parser profile, abnormality analysis, parsed payload, and locator trail)
...
- `report_json` (LONGTEXT, NULLABLE)
- `created_at` (TIMESTAMP)

### accumul8_ai_conversations
- `id` (BIGINT, PRIMARY KEY, AUTO_INCREMENT)
- `owner_user_id` (INT, FOREIGN KEY -> users.id)
- `title` (VARCHAR)
- `system_prompt` (LONGTEXT, NULLABLE)
...
### accumul8_ai_conversation_events
- `id` (BIGINT, PRIMARY KEY, AUTO_INCREMENT)
- `conversation_id` (BIGINT, FOREIGN KEY -> accumul8_ai_conversations.id)
- `owner_user_id` (INT, FOREIGN KEY -> users.id)
...
### accumul8_message_board_messages
- `id` (BIGINT, PRIMARY KEY, AUTO_INCREMENT)
- `owner_user_id` (INT, FOREIGN KEY -> users.id)
- `actor_user_id` (INT)
- `source_kind` (VARCHAR)
- `message_level` (VARCHAR: 'info', 'success', 'warning', 'error')
- `title` (VARCHAR)
- `body_text` (TEXT)
- `meta_json` (LONGTEXT, NULLABLE)
- `message_fingerprint` (CHAR(64), normalized dedupe hash for repeat alerts)
- `duplicate_count` (INT, how many times the same alert has been resurfaced)
- `is_acknowledged` (TINYINT(1))
- `acknowledged_at` (DATETIME, NULLABLE)
- `acknowledged_by_user_id` (INT, NULLABLE)
- `created_at` (TIMESTAMP)
